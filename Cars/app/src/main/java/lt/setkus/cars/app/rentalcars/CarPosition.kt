...
data class CarPosition(val id: String, val title: String, val latitude: Double, val longitude: Double)

val carPositionMapperFromDomain: (List<Car>) -> List<CarPosition> = { cars ->
    cars.map { CarPosition(it.id, it.modelName, it.latitude, it.longitude) }
}
